...
def check_privileges():
    """Vérifie si l'utilisateur a les droits admin/root."""
    if os.name == 'nt':
        if not is_admin_windows():
            print("[-] Ce script nécessite des privilèges administrateur sur Windows.")
            sys.exit(1)
    else:
        if os.geteuid() != 0:
            print("[-] Ce script nécessite des privilèges root (sudo).")
            sys.exit(1)
